<?php
/*
Plugin Name: Gravity Forms Email Address Blacklist
Plugin URI: http://www.pagegwood.com
Description: This plugin adds the ability to set a blacklist of email addresses on the email field in gravity forms. Modified version of plugin that was originally created by Hall Internet Marketing (Tim Howe) to block certain email domains.
Version: 1.0
Author: Page Wood
...
if (class_exists("GFForms")) {
    ...
    class GFEmailBlacklist extends GFAddOn {
      ...
      protected $_title = "This plugin adds the ability to set a blacklist of emails on the email field in gravity forms.";
      ...
      function form_submit_button($button, $form){
          ...
          return $button;
			...
			function gf_emailblacklist_field_tooltips($tooltips){
			   $tooltips["form_field_email_blacklist"] = "<h6>Email Blacklist</h6> Please enter a comma separated list of emails you would like to block from submitting. Enter 'none' if you would like to override the default blacklist settings.";
			...
			function gf_emailblacklist_validation($validation_result) {
				...
				foreach($form['fields'] as &$field) {
					...
					//get the email from user enterd email
					$email = rgpost("input_{$field['id']}");

					//collect banned emails from backend and clean up
					if( !empty($field["email_blacklist"]) ){ //collect per form settings
						$ban_emails = $field["email_blacklist"];
									var_dump($ban_emails);
					}elseif( get_option('gravityformsaddon_'.$this->_slug.'_settings') ){ //collect default settings
						$ban_emails = get_option('gravityformsaddon_'.$this->_slug.'_settings');
						$ban_emails = $ban_emails['default_emailblacklist'];
					}else{
						//no else value
					}

					//create array of banned emails
					$ban_emails = explode(',',$ban_emails);
					$ban_emails = array_map('trim',$ban_emails);

					// if email is valid OR if the email field is empty, skip
					if(!in_array($email, $ban_emails) || empty($email))
						continue;
					...
					if( !empty($field["email_blacklist_validation"]) ) {
						...
						$validation_message = sprintf(__('Sorry, <strong>%s</strong> email accounts are not eligible for this form.'), $email);
					}
	        ...
	        return array(
	            array(
	                ...
	                "fields" => array(
				                ...
				                "tooltip" => "<h6>Email Blacklist</h6> Please enter a comma separated list of emails you would like to block from submitting their email. These setting can be overwritten on a per field basis",
				                "class"   => "medium"
				            ),
										array(
	                      "label"   => "Error Message",
